$(function() {
	...
	delQuoteModel: function(quote) {
	    this.quotes.remove(this.quotes.indexOf(quote));
	    delete this.storage[quote.id];
	    this.storage['order'].remove(this.storage['order'].indexOf(quote.id));
	...
	saveQuoteModel: function(quote_object) {
		...
		this.storage['order'] = [quote_object.id];
	    } else {
		this.storage['order'].push(quote_object.id);
	    }
	    localStorage.setItem('store', JSON.stringify(this.storage));
	},
	...
	render: function() {
	    ...
	    if (!sample) {
		el.setAttribute('class', 'quote_wrap cursor');
	    } else {
		el.setAttribute('class', 'quote_wrap');
	    }
	    el.setAttribute('id', this.model.id);
	    this.elements.el = $(el);
	...
	renderTags: function() {
		...
		for (var attr in attrs) {
			...
			for (var j = 0; j < attrs[attr].length; j++) {
			    ...
			    a.click(function() {
				_this.model.filter($(this).data('type'), $(this).data('attr'),
						   $(this).data('attr_value'));
			    });
			}
		    }
		}
	...
	var view = new CollectionView(model, new CollectionController(model),
				      {'collection' : $('#quotes'), 
				       ...
				       'sample': $('#sample')});
	if (sample) {
	    $.getJSON("js/sample.json", function(json) {
		model.storage = json;
		view.load();
	    });
	} else {
	    model.storage = JSON.parse(localStorage.getItem('store')) || {};
	    $("#quotes").sortable({
		update: function(event, ui) {
		    var data = $(this).sortable('toArray');
		    model.storage['order'] = data;
		    localStorage.setItem('store', JSON.stringify(model.storage));
		}
	    });
	    view.load();
	}
